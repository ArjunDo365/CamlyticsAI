...
export abstract class CommonHelper {
    ...
    public static SetUserData(name: string, value: any, IsUserData: boolean = false, JsonFormat: boolean = true) {
        if (IsUserData) {
            if (JsonFormat) {
    ...
    public static parseDescription = (content: any) => {
        ...
        if (typeof content === 'string') {
            try {
                ...
                if (Array.isArray(parsed) && parsed[0]?.type && parsed[0]?.value !== undefined) {
    ...
    public static renderFlutterQuillBlocks = (blocks: any) => {
        return blocks
            .map((block: { type?: string; value?: any }) => {
                if (block.type === 'text') {
    ...
    public static async PdfDownloadAsBlob(response: any, view: boolean = false, filename: string = '') {
        try {
            ...
            if (view) {
                ...
                const contentDisposition = response.headers?.get ? response.headers.get('content-disposition') : response.headers?.['content-disposition'];
